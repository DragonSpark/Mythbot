...
namespace DragonSpark.Mythbot.Environment
{
	...
	public sealed class ServiceConfiguration : Command<ConfigureParameter>, IServiceConfiguration
	{
		...
		ServiceConfiguration() : base(Start.An.Instance(RegisterOption<RelayerSettings>.Default)
		                                   .Then()
		                                   .Terminate(Start.An.Instance(Configuration.Default.Promote())
		                                                   .Then(DefaultServiceConfiguration.Default)
		                                                   .Selector())) {}
	}

	sealed class Configuration : ICommand<IServiceCollection>
	{
		...
		public void Execute(IServiceCollection parameter)
		{
			parameter.AddHttpClient<Destination>()
			         ...
			         .AddSingleton<DevelopmentModeProgram>();
		}
	}
	...
	sealed class RegisteredSource : ISelect<LaunchDarkly.EventSource.Configuration, IEventSource>
	{
		...
		public IEventSource Get(LaunchDarkly.EventSource.Configuration parameter)
		{
	...
	sealed class Relayer : FixedSelection<LaunchDarkly.EventSource.Configuration, IEventSource>
	{
		[UsedImplicitly]
		public Relayer(RegisteredSource source, RelayerSettings settings)
			: base(source,
			       new LaunchDarkly.EventSource.Configuration(settings.Source, delayRetryDuration: TimeSpan.Zero)) {}
	}

	public sealed class DevelopmentModeProgram : ISelect<IHost, Task>
	{
		readonly Func<IEventSource> _source;

		public DevelopmentModeProgram(Func<IEventSource> source) => _source = source;
